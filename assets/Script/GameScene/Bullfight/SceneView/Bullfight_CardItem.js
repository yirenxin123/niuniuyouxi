...
cc.Class({
    ...
    sendCardAnimation : function (cardlist) {
        ...
        for (var cardcount = 0;cardcount < 5 && !cc.game.isPaused();cardcount++)
        {
            ...
            var startPos = new cc.p(endPos.x - (endPos.x + this.CardNode.position.x + this.node.position.x),
                endPos.y - (endPos.y + this.CardNode.position.y + this.node.position.y ));
    ...
    openCardAnimation : function (cardlist, index1, index2) {
    ...
    setPlayerBetCount : function (count) {
        this.node.active = true;
